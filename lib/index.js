...
var Reader = function (path, options){
	...
	if (options.highWaterMark < 1){
		throw BinaryReaderError ("Invalid highWaterMark");
	}
...
Reader.prototype._open = function (cb){
	...
	fs.open (this._path, "r", function (error, fd){
		...
		fs.fstat (fd, function (error, stats){
			if (error) return cb (error);
			if (!stats.isFile ()){
				return cb (BinaryReaderError ("The path is not a file"));
			}
			me._size = stats.size;
...
Reader.prototype.cancel = function (){
	if (!this._q) throw BinaryReaderError ("The reader is closed");
...
Reader.prototype.read = function (bytes, cb){
	if (!this._q) throw BinaryReaderError ("The reader is closed");
	if (bytes < 1) throw BinaryReaderError ("Must read one or more bytes");
	...
	this._q.push (function (done){
		...
		if (!error){
			if (cb.length === 3){
				...
				this.pause ();
				var q = this;
				cb (bytesRead, buffer, function (){
					if (me._cancelled) throw BinaryReaderError ("Don't call the " +
							"callback if you cancel() the reader");
					q.resume ();
				});
			}else{
				cb (bytesRead, buffer);
...
Reader.prototype._seek = function (offset, whence, cb){
	...
	if (this._p < 0){
		return cb (BinaryReaderError ("The seek pointer must contain a positive " +
				"value"));
	}
